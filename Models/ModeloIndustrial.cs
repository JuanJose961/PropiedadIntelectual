...
namespace GISMVC.Models
{
    public class ModeloIndustrial
    {
        ...
        public static ModeloIndustrial GetById(int id)
        {
            ModeloIndustrial res = new ModeloIndustrial();
            try
            {
                ...
                var errores = "";
                if (da.Cons_proc_ModeloIndustrialById(id, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        int idx = 0;
                        ...
                        res.dibujo = RLArchivo.GetFromModeloIndustrial(res.id, "Dibujo");
                        res._explicacion = RLArchivo.GetFromModeloIndustrial(res.id, "Explicación");
                    }
                }
                else
                {
        ...
        public static List<ModeloIndustrial> Get(string id_usuario = "", int activo = -1)
        {
            List<ModeloIndustrial> res = new List<ModeloIndustrial>();
            try
            {
                DataAccess da = new DataAccess();
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.Cons_proc_ModeloIndustrial(out dt, out errores,id_usuario, activo))
                {
                    if (dt.Rows.Count > 0)
                    {
        ...
        public static RespuestaFormato Crear(ModeloIndustrial modelo)
        {
            ...
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.INS_proc_ModeloIndustrial(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato Actualizar(ModeloIndustrial modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_proc_ModeloIndustrial(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato ActualizarActivo(ModeloIndustrial modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_proc_ModeloIndustrial_Activo(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
