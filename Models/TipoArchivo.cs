...
namespace GISMVC.Models
{
    public class TipoArchivo
    {
        ...
        public static TipoArchivo GetById(int id)
        {
            TipoArchivo res = new TipoArchivo();
            try
            {
                ...
                var errores = "";
                if (da.Cons_TipoArchivoById(id, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        int idx = 0;
        ...
        public static List<TipoArchivo> Get(int activo = -1)
        {
            List<TipoArchivo> res = new List<TipoArchivo>();
            try
            {
                DataAccess da = new DataAccess();
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.Cons_TipoArchivo(out dt, out errores, activo))
                {
                    if (dt.Rows.Count > 0)
                    {
        ...
        public static RespuestaFormato Crear(TipoArchivo modelo)
        {
            ...
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.INS_TipoArchivo(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato Actualizar(TipoArchivo modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_TipoArchivo(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato ActualizarActivo(TipoArchivo modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_TipoArchivo_Activo(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
