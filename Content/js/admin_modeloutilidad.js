...
function Editar(id) {
    ...
    var sended_url = services_url + "SelectModeloUtilidadById";
    $.ajax({
        type: "POST",
        url: sended_url,
...
function Confirma01() {
    if (ValidaUpdate01() != false) {
        if (catalogo_actual.id == 0) {
            catalogo_actual.tipo_solicitud = 5;
            catalogo_actual.tipo_solicitud_nombre = "Modelo Utilidad";
        }
...
function ConfirmaEliminar01() {
    ...
    var sended_url = services_url + "UpdateModeloUtilidadActivo";
    $.ajax({
        type: "POST",
        url: sended_url,
        data: JSON.stringify(catalogo_actual),
...
function Enviar(id, tipo) {
    ...
    if (propiedad > 0) {
        window.location = horturl+'PI/RegistroMarca?propiedad=' + propiedad + '&solicitud=' + id;
    } else {
