...
namespace GISMVC.Models
{
    public class ConvenioLicencia
    {
        ...
        public static ConvenioLicencia GetById(int id)
        {
            ...
            try
            {
                ...
                var errores = "";
                if (da.Cons_ConvenioLicenciaById(id, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        int idx = 0;
                        ...
                        res.licenciatario = Int32.Parse(row[idx].ToString()); idx++;
                        res.licenciatario_nombre = row[idx].ToString(); idx++;
                        res.licenciante = Int32.Parse(row[idx].ToString()); idx++;
                        res.licenciante_nombre = row[idx].ToString(); idx++;
                        ...
                        {
                            res.solicitud_permalink = Utility.hosturl + "PI/ConvenioLicenciaDocumento?id=" + HttpUtility.UrlEncode(funcion.Encriptar(res.id.ToString())) + "&tp=" + HttpUtility.UrlEncode(funcion.Encriptar("solicitud"));
                        }
                        if (res.contrato_nombre != "")
                        {
                            res.contrato_permalink = Utility.hosturl + "PI/ConvenioLicenciaDocumento?id=" + HttpUtility.UrlEncode(funcion.Encriptar(res.id.ToString())) + "&tp=" + HttpUtility.UrlEncode(funcion.Encriptar("contrato"));
                        }
                        if (res.oficio_nombre != "")
                        {
                            res.oficio_permalink = Utility.hosturl + "PI/ConvenioLicenciaDocumento?id=" + HttpUtility.UrlEncode(funcion.Encriptar(res.id.ToString())) + "&tp=" + HttpUtility.UrlEncode(funcion.Encriptar("oficio"));
                        }
        ...
        public static List<ConvenioLicencia> Get(int activo = -1)
        {
            List<ConvenioLicencia> res = new List<ConvenioLicencia>();
            try
            {
                DataAccess da = new DataAccess();
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.Cons_ConvenioLicencia(out dt, out errores, activo))
                {
                    if (dt.Rows.Count > 0)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            int idx = 0;
                            var row = dt.Rows[i];
                            var item = new ConvenioLicencia();
                            item.id = Int32.Parse(row[idx].ToString()); idx++;
                            item.licenciatario = Int32.Parse(row[idx].ToString()); idx++;
                            item.licenciatario_nombre = row[idx].ToString(); idx++;
                            item.licenciante = Int32.Parse(row[idx].ToString()); idx++;
                            item.licenciante_nombre = row[idx].ToString(); idx++;
        ...
        public static RespuestaFormato Crear(ConvenioLicencia modelo)
        {
            ...
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.INS_ConvenioLicencia(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato Actualizar(ConvenioLicencia modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_ConvenioLicencia(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato ActualizarActivo(ConvenioLicencia modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_ConvenioLicencia_Activo(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato ActualizarDocumento(ConvenioLicencia modelo, int tipo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_proc_ConvenioLicenciaDocumento(modelo, tipo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
