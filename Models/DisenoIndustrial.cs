...
namespace GISMVC.Models
{
    public class DisenoIndustrial
    {
        ...
        public static DisenoIndustrial GetById(int id)
        {
            DisenoIndustrial res = new DisenoIndustrial();
            try
            {
                ...
                var errores = "";
                if (da.Cons_proc_DisenoIndustrialById(id, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        int idx = 0;
                        ...
                        res.dibujo = RLArchivo.GetFromDisenoIndustrial(res.id, "Dibujo");
                        res._explicacion = RLArchivo.GetFromDisenoIndustrial(res.id, "Explicación");
                    }
                }
                else
                {
        ...
        public static List<DisenoIndustrial> Get(string id_usuario = "", int activo = -1)
        {
            List<DisenoIndustrial> res = new List<DisenoIndustrial>();
            try
            {
                DataAccess da = new DataAccess();
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.Cons_proc_DisenoIndustrial(out dt, out errores,id_usuario, activo))
                {
                    if (dt.Rows.Count > 0)
                    {
        ...
        public static RespuestaFormato Crear(DisenoIndustrial modelo)
        {
            ...
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.INS_proc_DisenoIndustrial(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato Actualizar(DisenoIndustrial modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_proc_DisenoIndustrial(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato ActualizarActivo(DisenoIndustrial modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_proc_DisenoIndustrial_Activo(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
