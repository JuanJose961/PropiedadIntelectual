...
namespace GISMVC.Models
{
    public class Obra
    {
        ...
        public string rama_nombre { get; set; }
        ...
        public static Obra GetById(int id)
        {
            Obra res = new Obra();
            try
            {
                ...
                var errores = "";
                if (da.Cons_proc_ObraById(id, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        int idx = 0;
                        ...
                        res.nombre_obra = row[idx].ToString(); idx++;
                        res.nombre = row[idx].ToString(); idx++;
                        res.rama = Int32.Parse(row[idx].ToString()); idx++;
                        ...
                        res.rama_nombre = row[idx].ToString(); idx++;
                        ...
                        res.obra = RLArchivo.GetFromObra(res.id, "Obra");
        ...
        public static List<Obra> Get(int activo = -1)
        {
            ...
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.Cons_proc_Obra(out dt, out errores, activo))
                {
                    if (dt.Rows.Count > 0)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            item.nombre_obra = row[idx].ToString(); idx++;
                            item.nombre = row[idx].ToString(); idx++;
                            item.rama = Int32.Parse(row[idx].ToString()); idx++;
                            ...
                            item.rama_nombre = row[idx].ToString(); idx++;
        ...
        public static RespuestaFormato Crear(Obra modelo)
        {
            ...
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.INS_proc_Obra(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato Actualizar(Obra modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_proc_Obra(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
        ...
        public static RespuestaFormato ActualizarActivo(Obra modelo)
        {
            RespuestaFormato res = new RespuestaFormato();
            try
            {
                ...
                var dt = new System.Data.DataTable();
                var errores = "";
                if (da.UPD_proc_Obra_Activo(modelo, out dt, out errores))
                {
                    if (dt.Rows.Count > 0)
                    {
                        var row = dt.Rows[0];
